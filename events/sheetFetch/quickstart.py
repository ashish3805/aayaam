...
def main():
    ...
    if not values:
        ...
        for row in values:            
            ...
            desc = desc.replace("\n","<br>")
            reg = row[8]
            #<a href="',reg,'" class="btn waves-effect waves-light red darken-1" >Register</a>
            #<a href="',link,'" class="btn aves-effect waves-light blue darken-1" >Website</a>
            if len(reg)>0:
                reg = "".join(('<a target="_blank" href="',reg,'" class="btn waves-effect waves-light red darken-1" >Register</a>'))
            if len(link)>0:
                link = "".join(('<a target="_blank" href="',link,'" class="btn aves-effect waves-light blue darken-1" >Website</a>'))
            t =  "".join(('<div class="col s12 m4 l4"><div class="center promo promo-example"><i class="material-icons">location_on</i><br><h5 class="promo-caption">Venue</h5>                           <p class="light center">',venue,'</p>                        </div>                    </div>                    <div class="col s12 m4 l4">                        <div class="center promo promo-example">                            <i class="material-icons">call</i>                           <br><h5 class="promo-caption">',name,'</h5>                            <p class="light center">',contact,'<br>',email,'</p>                        </div>                    </div>                    <div class="col s12">                       <br> <p> ',desc,'</p>  <br>                  </div>                </div>    <!--button secton-->           <div class="row">    <div class="col s6" style="text-align:right">  ',reg,'  </div><div class="col s6" style="text-align:left">   ',link,'             </div>            </div><!--utoon section ends-->        </div>        </div>    </div></body><script src="js/jquery-2.1.1.js"></script><script src="js/main.js"></script><script src="event-details/min/plugin-min.js"></script><script src="event-details/min/custom-min.js"></script><!-- Resource jQuery --></body></html>')).encode('utf-8').strip()  
